function [rates] = sweep_anticor(w,gang,surface,step_size,rectify,speed,t,time,active_whisker)
...
for a = 1:w;
    for b = 1:w;
        ...
        if any(wh==active_whisker)
            if wh==5;
            stim = [(1/speed)*(whiskers(a,b)*((speed)*cosd(mod((t+180),360))));(1/speed)*(whiskers(a,b)*((speed)*sind(mod((t+180),360))))];
            stims(:,wh) = stim;
            else
