...
# Copied from http://tkinter.unpythonic.net/wiki/TileWrapper

####November 2006: Posted by Kevin Walzer, kw@codebykevin.com. Based on Tile wrapper by Martin Franklin. This version updates the wrapper to reflect changes in Tile commands, and adds support for Tile-based frames (ttk::frame). Freely reusable.

####December 2007: Minor updates to check for Tk 8.5: call 'package require tile' only if Tk < 8.5.--KW 


# CEBALERT: need to clean this up! param module should not depend on
# Tkinter.
try:
...
if __name__=="__main__":
    ...
    ##check for version--only call 'package require tile' if Tk < 8.5
