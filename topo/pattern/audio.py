...
class Audio(OneDPowerSpectrum):
    ...
    filename = Filename (
                            ...
                            constant = True,
                            precedence = 0.9,
                            doc = """File path (can be relative to Topographica's base path) to an
                                  audio file.  The audio can be in any format accepted by
                                  pyaudiolab, e.g. WAV, AIFF, or FLAC."""
                        )

    seconds_per_timestep = param.Number (
                                            default = 1.0,
                                            constant = True,
                                            doc = """ Number of seconds represented by 1 simulation 
                                                  time step."""
                                        )

    sample_window = param.Number (
                                    default = 2.0,
                                    constant = True,
                                    doc = """ The length of interval of the signal (in seconds) on which
    ...
    def __init__(self, **params):
        ...
        self._initialize_window_parameters (
                                              signal = self._source.read_frames(self._source.nframes, dtype = numpy.float32),
