...
def weighted_sample(seq,weights=[]):
    """
    Selects randomly from seq.  weights, if given, should
    be an sequence, the same length as seq, as would be passed to
    weighted_sample_id()
    """
    if not weights:
...
def weighted_sample_idx(weights):
    """
    Returns an integer in range [0,len(weights)), generated by
    sampling the discrete distribution represented by the sequence of
    weights.  NOTE that  the weights need not sum to unity, but can
    contain any non-negative values. e.g. [1 1 1 100] is a valid set
    of weights.  NOTE: doesn't work with numpy.array.flat iterators,
    you must use numpy.array.ravel() instead.
    """
    total = sum(weights)
    if total == 0:
...
def idx2rowcol(idx,shape):
    """
    Given a flat matrix index and a 2D matrix shape,
    return the (row,col) coordinates of the index.
    """
    assert len(shape) == 2
    rows,cols = shape
...
def rowcol2idx(r,c,shape):
    """
    Given a row, column, and matrix shape,
    return the corresponding index into the flattened
    (raveled) matrix.
    """
    assert len(shape) == 2
    rows,cols = shape
...
def centroid(pts,weights):
    """
    Returns the centroid of a weighted set of points as an array.
    pts = an array of points, one per row.
