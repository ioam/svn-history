...
<P>The design of Topographica is in terms of hierarchical sets of
...
<P>When adding a new method, or changing the semantics of an old one,
it is absolutely crucial to think about where in the hierarchy the new
...
<P>For instance, consider a hierarchy containing geometric objects for
a drawing program: ProgramObject, Shape, Rectangle.  If you realize
when working on Rectangle that it needs a resize() method, then please
stop and think before adding the operation directly to Rectangle.  In
this case, any Shape can be resized, so resize() should be a method of
...
<P>In any case, when deciding where to add the code, be sure to think
...
<P>It is also extremely important to think about the appropriate name
...
<P>One way to make sure that you are using appropriate language is to
...
<P>Such naming and documentation issues may seem trivial, but please
...
reader is trying to solve.  What the reader wants to do is undoubtedly
different from the specific case the programmer originally had in
mind, and only if everything is written at the appropriate level of
generality will it be clear whether the code covers the intended usage
or not.

<P>Note that there can be serious consequences for inappropriately
...
Such extra functions make everyone's lives more difficult, because they
