...
Orientation map scaling:
    ...
    the scaling equations from \emcite{bednar:neuroinformatics04}.  The size
...
\cite{bednar:neuroinformatics04}. 
...
\cite{kaas:ccortex97,vanessen:vres01}.  These maps contain systematic
...
\cite{blasdel:orientation,merzenich:jnp75,weliky:nature96}.
...
(For a review of this class of models, see \cite{swindale:network96}.)
...
\cite{hines:nc97} and GENESIS \cite{bower:genesisbook98}, primarily
...
such as PDP++ \cite{oreilly:book00} and Matlab (www.mathworks.com),
...
This paper introduces the \topo{} map-level simulator, which is
designed to make it practical to simulate large-scale, detailed models
of topographic maps.  \topo{} is designed to complement the existing
...
containing millions or tens of millions of connections.  \topo{} is
...
\topo{} is an open source project, and binaries and source code will
be freely available through the internet at \texttt{topographica.org}.
In the sections below, we describe the models and modeling approaches
supported by \topo{}, how the simulator is designed, and how it can
be used to advance the field of computational neuroscience.

\beforesection{}
\section{Scope and modeling approach}
\aftersection{}

\newlength\figwidth\setlength\figwidth{\ifscribbledraft{1.6}\textwidth}%
%
\begin{figure}
  \begin{minipage}{0.60\figwidth}
    \centering
    \pdfpic[\psmono]{\textwidth}{!}{images/generic-cortical-hierarchy-model}{pdf}
  \end{minipage}\ifscribbledraft{\hfill\\}
  \hfill
  \begin{minipage}{0.35\figwidth}
  \titledcaption[\psmono]{fg:generic-cortical-hierarchy-model}
  {Topographica models}{%
    This figure shows a sample \topo{} model of the early visual
    system \cite{bednar:nc03,bednar:neurocomputing03}.
    In \topo{}, models are composed of interconnected sheets of neurons.  
...
\end{figure}%
%%
Figure~\ref{fg:generic-cortical-hierarchy-model} illustrates the types
of models supported by \topo{}.  The models focus on topographic maps
...
speech processing, and motor control.  \topo{} is intended to support
...
\emph{sheet} of neurons, rather than a neuron or a part of a neuron.
Conceptually, a sheet is a continuous, two-dimensional area (as in
\cite{amari:topographic,roquedasilvafilho:phd92}), which is typically
...
\cite{bednar:neuroinformatics04} for more details on model scaling.)
...
\topo{} consists of a graphical user interface (GUI), scripting
...
Figure~\ref{fg:lissom-screenshot} shows examples of the visualization
types currently supported.  This figure is a screenshot from a
prototype version of \topo{}, available for download at
\texttt{topographica.org}.
%
\begin{figure}%
  \pdfpic[\pscolor]{\textwidth}{!}{images/lissom.4.0_linuxscreenshot2_white\imagesuffix}{png}
  \titledcaption[\psmono]{fg:lissom-screenshot}
  {Software screenshot}{%
    This image shows a sample session of a prototype
    version of \topo{} that is available freely at \texttt{topographica.org}.  Here the
    ...
    figure~\ref{fg:generic-cortical-hierarchy-model}.  The window at
    ...
    an \mtimes{8}{8} and \mtimes{4}{4} sampling of the V1 neurons are
    shown in the ``Weights Array'' windows on the right; most neurons
    are selective for Gabor-like patches of oriented lines.  The
    lateral connections for an \mtimes{8}{8} sampling of neurons are
    ...
    \topo{} is well suited for it.  See \texttt{topographica.org} for
    a color version of this figure.}
  \vspace*{4ex}% To give more space around the figure
\end{figure}%
...
section~\ref{s:analysis-and-validation} will describe the analysis and
...
section~\ref{s:scripting}.



The first full release of \topo{} is scheduled for late 2004, and will include
...
Using the tools provided by \topo{}, we expect that neuroscientists
...
Several releases of \topo{} are planned over the next few years,
...
The \topo{} simulator fills an important gap between existing software
