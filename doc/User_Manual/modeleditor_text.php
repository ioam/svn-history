...
console. Select the Simulation menu in the console's top menu bar, and
choose the model editor. A new window will appear with a toolbar on
...
<p>There are two main classes of component used to define a model:
Sheets and Projections.

<p>Sheets are the main computational unit in Topographica and are
...
neurons are on a sheet. This can be achieved by right clicking in the
main window and selecting Toggle Density Grid from the Sheet options
menu. The other option, Toggle Activity, allows you to display the
actual activity of the sheets. If you only want to view the activity
of one sheet then right click on the particular sheet and select
Activity from the Change View menu. The activity of each neuron of the
sheet is plotted onto the parallelogram, so the density of the sheet
will determine how detailed the image appears.

<p>Although the sheets are the main units, they are each independent
of one another, and thus they need some form of connection to be a
...
projection, and selecting Fixed Size (to turn off the size scaling) or
Line (to connect Sheets using a plain line rather than a rendition of
a ConnectionField).
...
<p>The name of each sheet and projection is displayed beside its
...
from the same sheet is marked d) and a density grid and activity plot
can be viewed on e) and f) respectively:
<center>
...
<h3>Toolbar</h3>
<p>On the left hand side of the editor is a toolbar that sets the mode
...
<h3>The Work Area</h3>
<p>The main part of the editor is the work area (canvas), this
displays a representation of the model that Topographica is currently
using. If you haven't opened a model, this area will be empty. The
first thing you may want to do is add a sheet to the model. Click on
...
<p>To allow responses to propagate through your model, you need to
connect the sheets together. Click on the connection pull down and a
list of all the existing types of projections will be presented. This
list collects the pre-defined projections available to Topographica
and is described later. Select the type of projection you want, for
example, for a standard connection with a receptive field select a
CFProjection. The canvas is now in connection mode, this means it will
expect you to drag the mouse from one sheet to another (or to the same
sheet for feedback connections). To do this click and hold the mouse
over the sheet you want the connection to be from. Then, while holding
the mouse button down, move the mouse to the sheet you want to be
connected to. Release the mouse button:
...
representation for the connection as shown above. If you have selected
a sheet, but decide you don't want a connection, release the mouse in
an area without any other components. To connect a sheet to itself,
simply select the sheet and release the mouse while it is still over
the same sheet. 

<h3>The Predefined Components</h3>

<p>Topographica supplies a library of components, composed of both
...
the reference manual, or look at the online help for an object in the
Parameters window (below). The Library section is divided into several
different parts, so you can view all the possible sheets or
projections available to you and use their descriptions to understand
how you can use them in your model.
...
and these additions are automatically available in the model
editor. If you decide to extend any of the components, simply save the
files in the same directory as the others of that type (or load it
...
a component has, right click on the component's representation in the
work area. This will bring up a window with the component's name at
the top and several entries underneath. You will notice that these
entries vary in style, for a description of the various types of
