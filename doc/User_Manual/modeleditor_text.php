...
Topographica allows you to view, manipulate or create a Topographica model. By combining pre-defined components and changing each component's parameters, you can produce useful models without any programming.  However, there are some limitations, such as only being able to save the model as a snapshot (which will not always be readable in future versions of Topographica), and not being able to declare some types of values (such as dynamic numbers) in the GUI.  As a result, the Model Editor is currently most useful for inspecting and modifying a model that you create with a script file.
...
<p>Import the model you wish to visualise and then open a model editor window (as described above). The model editor will automatically display the existing model.  The location of each Sheet will typically be random initially, but the Sheets can be rearranged using the Move tool.

<h3>Representations</h3>
<p>There are two main classes of component used to define a model: Sheets and Projections. 
...
<p>Although the sheets are the main units, they are each independent of one another, and thus they need some form of connection to be a functioning network. Projections form a uni-directional connection between sheets, allowing activity to propagate through a model. The projections allow each neuron on one sheet to be connected to a certain area on the other sheet; this area is called the neuron's ConnectionField. The default view plots a ConnectionField to scale.  Other views can be selected by right clicking on the desired projection, and selecting Fixed Size (to turn off the size scaling) or Line (to connect Sheets using a plain line rather than a rendition of a ConnectionField).

<p>The editor represents connections that join a sheet to itself as a dotted ellipse around the centre of the sheet. These projections allow each neuron on a sheet to be stimulated the previous activity within a localized area. A looped line is used as the line representation of the projection.

<p>The name of each sheet and projection is displayed beside its representation in the canvas. To find out the name of a projection that links a sheet to itself, hover the mouse over the representation. If it is not obvious which name is linked with an item, right click on the representation and select Properties. The name is displayed at the top of the window. To view other properties of a component refer to section <a href="#parameters">The Parameters</A>, which describes the properties window.
...
<p>Creating a new model for Topographica can be achieved in two ways. If you have programming experience, it is probably easiest to define a model in Python script. Example scripts are distributed with Topographica in the examples directory. The second way is to use the model editor to either create a brand new model, or manipulate an existing model, although not all operations can be specified in the Model Editor yet.
...
<p>To allow responses to propagate through your model, you need to connect the sheets together. Click on the connection pull down and a list of all the existing types of projections will be presented. This list collects the pre-defined projections available to Topographica and is described later. Select the type of projection you want, for example, for a standard connection with a receptive field select a CFProjection. The canvas is now in connection mode, this means it will expect you to drag the mouse from one sheet to another (or to the same sheet for feedback connections). To do this click and hold the mouse over the sheet you want the connection to be from. Then, while holding the mouse button down, move the mouse to the sheet you want to be connected to. Release the mouse button:
<center>
<IMG WIDTH="649" HEIGHT="378" SRC="images/editorconnectiondrag.png">
</center>
<p>If the target location is near to an existing sheet, this will form a connection between the two sheets and you will see the clear representation for the connection as shown above. If you have selected a sheet, but decide you don't want a connection, release the mouse in an area without any other components. To connect a sheet to itself, simply select the sheet and release the mouse while it is still over the same sheet.

<h3>The Predefined Components</h3>
<p>Topographica supplies a library of components, composed of both basic and specific sheets and projections. To understand what a particular component does you should consult the Library section of the reference manual, or look at the online help for an object in the Parameters window (below). The Library section is divided into several different parts, so you can view all the possible sheets or projections available to you and use their descriptions to understand how you can use them in your model.
...
<p>The Topographica team are regularly making additions to the library and these additions are automatically available in the model editor. If you decide to extend any of the components, simply save the files in the same directory as the others of that type (or load it into Python beforehand from any file) and your extension will automatically be available for use in the model editor.
...
<p>If you are looking at a sheet's parameters you will notice another menu under these buttons. This allows you to open the properties frame of any of the connections joined to that sheet. Simply select a connection from the list and its frame will open underneath.
<P>Help for most of the options can be obtained by hovering the mouse over the option's name.
Help for the entire object can be obtained by hovering over the object's name ('Surround' in this case).

<h3>Constants</h3>
<p>The Parameters section of the User Guide also details that a parameter can be declared as a constant. This means you cannot change it once the component has been made. To ensure consistency, the model editor must create the components as soon as you make them in the editor, so these values must already be set. To set constant values (as well as any other values) before instantiating the object, the editor provides a window 'New object parameters' when one of the object creation widgets (Sheet or Projection) is selected.  Values set in this window are used for any new object created with those widgets.
