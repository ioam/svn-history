...
<P>With a simulator written in a compiled programming language like
C/C++ or Java, it makes sense to ask what the simulator does and does
not support, because ordinary users are not expected to modify the
simulator source code.  Even when a custom scripting language is
supplied with the simulator, the scripting language is typically quite
limited, so the question of what that language supports is meaningful.

<P>However, because Topographica is based on a fully general-purpose
interpreted programming language (Python), in a trivial sense one
could argue that Topographica supports any possible model, and indeed,
any possible software program.  For Topographica, a more relevant
question to ask is 'how much effort and programming expertise does it
take to do what I want in Topographica?'.  The answer to this question
depends on the task, of course, but it can be broken down into the
following main levels, depending on how much of Topographica's code
you can utilize for your own purposes.  The list is ordered so that
the most general support, suitable for everyone but requiring the most
effort, is at the top, and the most specific support is at the bottom.
In Topographica, each successive level is designed to be completely
independent of all levels below it, in the strict sense that
everything in topo/ besides those files listed can be deleted with no
ill effects.  The useful levels include:
...
already solved), but the limitations governing what can be done at
that level increase.  To the extent that these limitations are
appropriate for what you want to model, Topographica will be an
appropriate tool.  If what you want to do does not fit into these
limitations, you will have to go up to higher levels in this list,
doing more of the implementation work yourself and gaining less from
what the Topographica developers have done.  If everything you are
doing ends up being implemented at level 1 above, then there is no
reason to use Topographica at all, except as an example of how to use
Python and C together.

<P>Note that different parts of any particular model may be at
different positions in this list.  For instance, even for a model that
