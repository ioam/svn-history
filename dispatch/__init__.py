...
class TaskSpecifier(param.Parameterized):
   ...
   def show(self):
      ...
      if self.dynamic: 
         logging.info('No data available to show remaining output of dynamic TaskSpecifier %s' %
...
class TaskLauncher(param.Parameterized):
    ...
    def commandline(cls, argsv): 
        ...
        that decorator will exit immediately after (prevents the calling
    ...
    def append_log(self, specs):
       """The log contains the tids and corresponding specifications used during launch
          with the specifications in json format. """
    ...
    def record_info(self):
       """ All Tasklaunchers should call this method to write the info
       file at the end of the launch.  The info file saves the full
       timestamp and launcher description. The file is written to the
       root_directory."""
    ...
    def limit_concurrency(self, elements):
        """ Helper function that breaks list of elements into chunks
...
class QLauncher(TaskLauncher):
    ...
    def qsub_args(self, override_options, cmd_args):
        ...
        opt_dict.update(list(override_options.items())) # Keep this way for OrderedDict
...
class review_and_launch:
    ...
    def __init__(self, launcher_class, output_directory=None, review=True, check_main=True):
       ...
       if not isinstance(launcher_class,type) \
               or not issubclass(launcher_class, TaskLauncher):
