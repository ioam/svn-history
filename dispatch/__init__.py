...
class TaskCommand(param.Parameterized):
   ...
   Launcher) but there are some important cases where the specification
...
class Launcher(param.Parameterized):
    ...
    def dispatch(self):
        """ The method that starts Launcher execution. Typically called by a
...
class QLauncher(Launcher):
    """
    Launcher that operates the Sun Grid Engine using default arguments
    ...
    qsub_flag_options = param.Dict(default={'-b':'y'}, doc="""
          Specifies qsub flags and their corresponding options as a dictionary. Valid
          values may be strings or lists of string.  If a plain Python dictionary is
          used, the keys are alphanumerically sorted, otherwise the dictionary is
          assumed to be an OrderedDict (Python 2.7+, Python3 or
          param.external.OrderedDict) and the key ordering will be preserved.
    ...
    def qsub_args(self, override_options, cmd_args):
        ...
        qsub_switches and qsub_flag_options parameters. Switches are returned first,
        ...
        OrderedDict). Otherwise the keys are sorted alphanumerically. Note that
...
class review_and_launch:
    ...
    would execute the whole script and therefore re-run the Launcher which
