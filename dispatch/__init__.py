...
class TaskSpecifier(param.Parameterized):
   """ The base class for all TaskSpecifiers. On each iteration, a Taskspecifier
       can returns a list of dictionaries (parameter specifications). The keys
       of each specification are the parameter names while the string values
       specify the values of the parameter that can be later built into
       commandline strings.
       ...
       For some search strategies (eg. hillclimbing), feedback is needed at
       runtime before the next sample of the parameter space is taken. In these
       cases, dynamic=True and the state of the TaskSpecifier is updated between
      ...
      the parameters we wish to manipulate between runs (informative) and
      constant settings (less informative).
   ...
   dynamic = param.Boolean(default=False, constant=True, doc='''
           ...
           updated via the update() method between iterations (ie. between calls
           to the next method). If all the specifications cannot be known ahead
           of time (eg. parameter search), dynamic must be set to True.''')
   ...
   rationale = param.String(default='',  allow_None=True, doc='''
     ...
     Note that rationale is one of the few TaskSpecifier parameters not declared
     constant''')
   ...
   def constant_keys(self):
      """ Returns the list of parameter names whose values vary as the TaskSpecifer is iterated.
          Note that constant_keys() + varying_keys() should cover the entire set of keys. """
      raise NotImplementedError

   def varying_keys(self):
      """ Returns the list of parameters whose values are constant as the TaskSpecifer is iterated.
          Whenever it is possible, keys should be sorted from slowest varying to fastest varying
          and sorted alphanumerically within groups varying at the same rate. """
      raise NotImplementedError

   def next(self):
      """ Called to get a list of specifications: dictionaries with parameter
   ...
   def update(self, data):
      """ Called to update the state of the iterator when dynamic= True.
          ...
          the parameter space. If the update fails or data is None,
          StopIteration should be raised."""
      raise NotImplementedError

   def schedule(self): 
      """ Specifies the expected number of specifications that will be returned
      on future iterations. This is simply a list of integers specifying the
      number of specifications that will be returned on each call to next. Only
      invoked if dynamic=True and returns None if scheduling is not possible.
   ...
   def _collect_by_key(self,specs):
       """ Returns a dictionary like object with the lists of values collapsed
           by their respective key. Useful to find varying vs constant keys and
           to find how fast keys vary."""
   ...
   def show(self):
      """ Convenience method to inspect the available values generated by the
          TaskSpecifier in human-readable format. When not dynamic, allows you
          to easily see how many specifications will be generated.

          When dynamic=True, as many specifications will be listed as possible
          before update is required."""

      copied = self.copy(); copied.print_level = 50
      ...
      if self.dynamic: 
         message = 'No data available to show remaining output of dynamic TaskSpecifier %s'
         print (message % self.__class__.__name__)

   def __str__(self): 
      ''' TaskSpecifiers are expected to have a succinct representation that is
          human-readable but correctly functions as a proper object
          representation (rationale may be omitted however).'''
      return repr(self)
   ...
   def _cartesian_product(self, first_specs, second_specs): 
      """ Takes the Cartesian product of the specifications. Result will contain
          N specifications where N = len(first_specs) * len(second_specs) and
          keys are merged.  Example: [{'a':1},{'b':2}] * [{'c':3},{'d':4}] =
          [{'a':1,'c':3},{'a':1,'d':4},{'b':2,'c':3},{'b':2,'d':4}]"""

      return  [dict(zip(s1.keys()+s2.keys(),s1.values()+s2.values())) 
...
class StaticSpecs(TaskSpecifier):
    ...
    def extract_log_specification(log_path):
       """ Parses the log file generated by a launcher and returns the
       corresponding tids and specification list. The specification list can be
       used to build a valid StaticSpecs object. The tid list can be useful for
       building tables and associates the specifications with the tasks run."""
    ...
    def __init__(self, specs, rationale='', fp_precision=4):
       self._specs = specs; self._exhausted = False
    ...
    def next(self):
       ...
       else: self._exhausted=True; return self._specs
    ...
    def _unique(self, sequence, idfun=repr):
       ...
       seen = {}; return [seen.setdefault(idfun(e),e) for e in sequence if idfun(e) not in seen]
...
class StaticConcatenate(StaticSpecs):
   ...
   def __init__(self, first, second, rationale=''):
      ...
      super(StaticConcatenate, self).__init__(first.copy().next() 
                                            + second.copy().next(), 
                                              rationale=rationale)
...
class StaticCartesianProduct(StaticSpecs):
   ...
   def __init__(self, first, second, rationale=''):
      ...
      specs = self._cartesian_product(first.next(), second.next())
...
class Spec(StaticSpecs): 
   """ Allows easy instantiation of a single specification using keywords. 
       Useful for instantiating constant settings before a cartesian product."""
...
class LinearSpecs(StaticSpecs):
   """ LinearSpecs generates a range of specifications with linearly
   interpolated numeric values to a certain floating-point precision."""
...
class ListSpecs(StaticSpecs):
   ...
   def __init__(self, spec_key, value_list, fp_precision=4, rationale=''):
      ...
      self._specs = []; self._exhausted = False
...
class DynamicConcatenate(TaskSpecifier):
   ...
   def __init__(self, first, second, rationale=''):
      ...
      self._exhausted = False; self._first_sent = False
      self._first_cached = None; self._second_cached = None
      if not first.dynamic: self._first_cached = first.copy().next()
      if not second.dynamic: self._second_cached = second.copy().next()
...
class TaskCommand(param.Parameterized):
   ...
   format of a single task specification to a particular command that needs
   ...
   TaskCommands should avoid all platform specific logic (this is the job of the
   TaskLauncher) but there are some important cases where the specification
   ...
   executable = param.String(default='python', constant=True, doc='''
       ...
       executable is a standard command that can be expected on the system path,
       this should be an absolute path to the executable. By default this
   ...
   def __init__(self, executable=None, **kwargs):
      ...
      if executable is None: 
         if sys.argv[0] == '-c':  executable = sys.executable
         if len(sys.argv) == 1:   executable = sys.executable
         else:                    executable =  os.path.abspath(sys.argv[0])
   ...
   def show(self, task_specifier, file_handle = sys.stdout , queue_cmd_only=False):
      ...
      if queue_cmd_only and not hasattr(self, 'queue'): 
         print "Cannot show queue: TaskCommand not queueable"; return
      elif queue_cmd_only: 
         full_string = 'Queue command: '+ ' '.join([pipes.quote(el) for el in self.queue('<tid>',info)])
      else:
         copied = task_specifier.copy(); copied.print_level = 0; full_string = ''
         enumerated = [el for el in enumerate(copied)]
         for (group_ind, specs) in enumerated:
...
class TaskLauncher(param.Parameterized):
    """ A TaskLauncher takes a name, a TaskSpecifier and TaskCommand in the
        constructor and launches the corresponding tasks appropriately when
        invoked.

        This TaskLauncher is a simple example that uses subprocess to launch the
        tasks. It is designed the illustrate the basic design and should be used
        a base class for more complex TaskLaunchers."""
    ...
    tag = param.String(default='', doc='''
           A very short, identifiable human-readable string that meaningfully
           describes the batch to be executed. Should not include spaces as it
           may be used in filenames.''')
    ...
    max_concurrency = param.Integer(default=2, allow_None=True, doc='''
         ...
         subprocess locally, two concurrent processes is the limit. Set to None
         for no limit (eg. for clusters)''')

    exit_callable = param.Callable(default=None, doc=''' 
         A callable that will be invoked when the TaskLauncher has completed all
         tasks. For example, this can be used to collect and analyse data generated across
         tasks (eg. reduce across maps in a RunBatchAnalysis), inform the user of
         completion (eg. send an e-mail) among other possibilities.''')

    timestamp = param.NumericTuple(default= (0, 0, 0, 0, 0, 0, 0, 0, 0), doc="""
        Optional override of timestamp (default timestamp set on dispatch call)
        in Python struct_time 9-tuple format.  Useful when you need to have a
        known root_directory path (see root_directory documentation) before
        launch. For example, you should store state related to analysis
        (eg. pickles) in the same location as everything else.""") 
    ...
    timestamp_format = param.String(default='%Y-%m-%d_%H%M', allow_None=True, doc=""" 
         ...
         datetime format. If None is used, timestamp is omitted from root directory.""")
     
    print_info = param.ObjectSelector(default='stdout',  objects=['silent', 'summary','stdout'], 
          doc='''The output verbosity of the launcher. Option 'silent' prints out
          nothing, 'summary' prints only summary and 'stdout'relays the stdout
          of the tasks.''')

    metric_loader = param.Callable(default=pickle.load, doc='''
         The function that will load the metric files generated by the task. By
         default uses pickle.load but json.load is also valid option. The
         callable must take a file object argument and return a python object. A
         third interchange format that might be considered is cvs but it is up
         to the user to implement the corresponding loader.''')

    @classmethod
    def commandline(cls, argsv): 
        ''' Class method to allow TaskLaunchers to be controlled from the
        commandline.  If commandline is processed, should return True - note
        that sys.exit() will be called immediately after (prevents the calling
        decorator from relaunching)'''
        return False


    def __init__(self, batch_name, task_specifier, task_command):

        super(TaskLauncher,self).__init__(task_specifier=task_specifier,
        ...
        """
        This is the root directory in which all files related to the launch are
        to be stored. This includes a launcher log file, an info file, a metrics
        folder full of metrics (dynamic TaskSpecifiers only) and is where -all-
        files generated by the executed tasks should reside, preferably organised
        appropriately into subdirectories.

        It is important to keep everything together for reasons of simplicity
        and for archival purposes. For more advanced launchers, (eg. working on
        clusters) this is where all logs of stdout/sdterr streams should be
        stored and where any stored state (typically pickle files) for the
        launcher or analysis should also be found.

        The root directory is always the <timestamp>-<batch-name> subdirectory of
        the param.normalize_path(). Without a prefix set, normalize_path is
        simply the current working directory. Setting the timestamp parameter
        from outside allows the root directory to be known before launch.

        Note that this parameter cannot be set explicitly and is only
        instantiated to a proper path once dispatch begins (ie. once a timestamp
        is guaranteed to be available).  To customize the root_directory, change
        the normalize_path prefix. In other words, import param and use
        param.normalize_path.prefix = <desired path>."""
    ...
    def record(self):
       """ All Tasklaunchers should call this method to write a log and and info
           file.  The log contains the tids and corresponding specifications
           used during launch with the specifications in json format. The info
           file saves the full timestamp and launcher description. The files are
           written to the root_directory."""

       with open(os.path.join(self.root_directory, ("%s.log" % self.batch_name)), 'w') as log: 
          lines = ['%d %s' % (tid, json.dumps(spec))  for (tid,spec) in self._spec_log]
          log.write('\n'.join(lines))

       with open(os.path.join(self.root_directory, ('%s.info' % self.batch_name)), 'w') as info:
          timestr = time.strftime("Start Date: %d-%m-%Y Start Time: %H:%M (%Ss)", self.timestamp)
          lines = [timestr, 'Batch name: %s' % self.batch_name, 'Description: %s' % self.description]
          info.write('\n'.join(lines))

    def _setup_launch(self):
        """ Method to be used by all launchers that prepares the rootdirectory
            and generate basic launch information for task specifiers to
            use. Prepends some basic information to the description, registers a
            timestamp and return a dictionary of useful information constant
            across all tasks."""
        ...
        if not os.path.isdir(metrics_dir) and self.task_specifier.dynamic: os.makedirs(metrics_dir)

        formatstr = "Batch '%s' of tasks specified by %s and %s launched by %s with tag '%s':\n" 
        classnames= [el.__class__.__name__ for el in [self.task_specifier, self.task_command, self]]
        self.description = (formatstr % tuple([self.batch_name]+ classnames + [self.tag]))+self.description
    ...
    def extract_metrics(self, tids, launchinfo):
        """ Method to extract the metrics generated by the tasks required to
            update the TaskSpecifier (if dynamic). Uses the metric loaded to
            extract the metrics of specified tids in the metrics subdirectory of
            the root directory. Convention is that metric files should always be
            prefixed with metric-<tid>- in this directory. Note the trailing '-'
            and if there are multiple files with same tid prefix, they will all
            be returned."""
        
        metrics_dir = os.path.join(self.root_directory, 'metrics')
        listing = os.listdir(metrics_dir)
        try:
           matches = [l for l in listing for tid in tids if fnmatch.fnmatch(l, 'metric-%d-*' % tid)]
           pfiles = [open(os.path.join(metrics_dir, match),'r') for match in matches]
           return [self.metric_loader(pfile) for pfile in pfiles]
        except:
           print "Cannot load required metric files. Cannot continue."
           return None # StopIteration should be raised by the TaskSpecifier.
           
    def limit_concurrency(self, commands):
       """ Helper function that breaks list of specifications into chunks
           (sublists) of size self.max_concurrency."""

       if self.max_concurrency is None: concurrency_limit = len(commands)
       else:                            concurrency_limit = self.max_concurrency
       return [commands[i:i+concurrency_limit] for i in range(0, len(commands), concurrency_limit)]

    def dispatch(self):
        """ The method that starts TaskLauncher execution. Typically called by a
        task decorator.  This could be called directly by the users but the risk
        is that if __name__=='__main__' is omitted, the launcher may rerun on
        any import of the script effectively creating a fork-bomb."""
        launchinfo = self._setup_launch()
        
        last_tid = 0; last_tids = []
        for groupspecs in self.task_specifier:

           tids = range(last_tid, last_tid+len(groupspecs)); last_tid += len(groupspecs)
           allcommands = [self.task_command(spec, tid, launchinfo) for (spec,tid) in zip(groupspecs,tids)]
            
           self._spec_log += zip(tids,groupspecs)
            
           for commands in self.limit_concurrency(allcommands):
              processes = [subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT) 
                           for cmd in commands]
              if self.print_info != 'silent': print "Group of %d subprocesses started..." % len(processes),
              if self.print_info == 'stdout': print '\n'.join([p.stdout.read() for p in processes])
              pids = [p.pid for p in processes]
              [os.waitpid(p,0) for p in pids]
              if self.print_info != 'silent': print "Completed."
              last_tids = tids[:]
...
class QLauncher(TaskLauncher):
    """
    TaskLauncher that operates the Sun Grid Engine using default arguments
    suitable for runnign on the Edinburgh Eddie cluster. Allows automatic
    parameter search strategies such as hillclimbing to be used where parameters
    used depend on previously generated results without blocking waiting on
    results.
    ...
    qsub_switches = param.List(default=['-V', '-cwd'], doc = """
          Specifies the qsub switches (flags without arguments) as a list of strings. By default
          the -V switch is used to exports all environment variables in the host environment to
          the batch job. The prefered method of specifying this parameter is to use the
          add_qsub_settings and set_qsub_settings methods. """)

    qsub_flag_options = param.Dict(default={'-b':'y'}, doc="""
          Specifies qsub flags and their corresponding options as a dictionary.
          If a plain Python dictionary is used, the keys are alphanumerically
          sorted, otherwise the dictionary is assumed to be an OrderedDict
          (Python 2.7+, Python3 or param.external.OrderedDict) and the key
          ordering will be preserved.

          By default the -b (binary) flag is set to 'y' to allow
          binaries to be directly invoked. Note that the '-' is added
          to the key if missing (to make into a valid flag) so you can
          specify using keywords in the dict constructor: ie. using
          qsub_flag_options=dict(key1=value1, key2=value2, ....)
        """)

    @classmethod
    def commandline(cls, args): 
        ''' Continues the execution of the launcher if argument
        'collate_and_launch' is last argument on the commandline. In this case,
        the root directory is expected to be the preceeding argument. This
        information allows the launcher.pickle file to be unpickled to resume
        launch.'''
        if args[-1] != 'collate_and_launch': return
    ...
    def __init__(self, batch_name, task_specifier, task_command):
        super(QLauncher, self).__init__(batch_name, task_specifier, task_command)
        self._launchinfo = None; self.schedule = None
        self.last_tids = []; self._spec_log = []
        self.last_tid = 0; self.last_scheduled_tid = 0; self.collate_count = 0
    ...
    def qsub_args(self, override_options, cmd_args):
        ...
        opt_dict={}; opt_dict.update(self.qsub_flag_options)
        opt_dict.update(override_options.items())

        if type(opt_dict) == dict:   # Alphanumeric sort if vanilla Python dictionary
            ordered_options = [(k, opt_dict[k]) for k in sorted(opt_dict.keys())]
        else: ordered_options =  opt_dict.items()
        
        # Add the - if missing (eg, if using dict constructor with keywords)
        ordered_options = [(k,v) if (k[0]=='-') else ('-%s' % k,v) for (k,v) in ordered_options]
        flattened_options = [el for kvpair in ordered_options for el in kvpair]
        return (['qsub'] + sorted(self.qsub_switches) 
                + flattened_options +  [repr(c) for c in cmd_args])

    def dispatch(self):
        """ Main entry point for the launcher. Collects the static information
        about the launch and sets up the stdout and stderr stream output
        ...
        log_path = os.path.join(param.normalize_path(), 
                               self.root_directory, "streams")
        output_path = os.path.join(log_path, "output")
        error_path = os.path.join(log_path, "error")
        self.qsub_flag_options['-o'] = output_path
        self.qsub_flag_options['-e'] = error_path

        dirs = [output_path, error_path]
        for d in dirs:
            try: os.makedirs(d)
            except: pass
        
        # Waiting till these directories exist (otherwise potential qstat error)
        while not all([os.path.isdir(d) for d in dirs]): pass
        self.collate_and_launch()

    def collate_and_launch(self):
        """ Method that collates the previous jobs and launches the next block
        of concurrent jobs. The launch type can be either static or dynamic
        ...
        try:   specs = self.task_specifier.next()
        except StopIteration:
            self.qdel_batch()
        ...
        self.last_tid += len(specs); self._spec_log += tid_specs
        ...
        output_dir = self.qsub_flag_options['-o']; error_dir = self.qsub_flag_options['-e'] 
        ...
        # Pickle launcher and exit if necessary.
        if (not self.task_specifier.dynamic) and (self.exit_callable is not None):
    ...
    def qsub_collate_and_launch(self, output_dir, error_dir, job_names):
        ...
        job_name = "%s_%s_collate_%d" % (self.batch_name, self.job_timestamp, self.collate_count)
        ...
        if self.print_info != 'stdout': print stdout
        if self.print_info != 'silent': print "Invoked qsub for next batch."
        return job_name

    def static_qsub(self, output_dir, error_dir, tid_specs):
        """ This method handles static TaskSpecifiers and cases where the
        dynamic TaskSpecifiers cannot be queued ahead of specification. """
        processes = []; job_names = []
        ...
        for (tid, spec) in tid_specs:
            ...
            popen_args = self.qsub_args({"-e":error_dir, '-N':job_name, "-o":output_dir}, cmd_args) 
            p = subprocess.Popen(popen_args, stdout=subprocess.PIPE)
            (stdout, stderr) = p.communicate()
            if self.print_info != 'stdout': print stdout
            processes.append(p)

        if self.print_info != 'silent': print "Invoked qsub for %d commands" % len(processes)
        if self.exit_callable is not None: 
...
class review_and_launch:
    ...
    TaskLauncher execution for the following reasons:
    ...
    would execute the whole script and therefore re-run the TaskLauncher which
    would cause a fork-bomb! This decorator only executes the Tasklauncher that
    is returned by the wrapped function if __name__=='__main__'
    
    2) Code in the script after TaskLauncher execution has been invoked is *not*
    ...
    proper way of executing code after the TaskLauncher exits."""

    def __init__(self, launcher_class, output_directory=None, review=True):
       self.review = review;  self.launcher_class = launcher_class
       self.output_directory = output_directory
       # The launcher classmethod 'commandline' is called before function invocation.
       if not isinstance(launcher_class,type) or not issubclass(launcher_class, TaskLauncher):
    ...
    def input_options(self, options, prompt='Select option'):
        ...
        if response not in options:  response = self.input_options(options, prompt)
        return response

    def review_task_specification(self, task_specifier):
        print self.section('TaskSpecifier')
        print "Type: %s (dynamic=%s)" % (task_specifier.__class__.__name__, task_specifier.dynamic)
        ...
        response = self.input_options(['y', 'n','quit'], '\nShow available taskspecifier entries?')
    ...
    def review_task_command(self, task_command, task_specifier, task_launcher):
        ...
        if isinstance(task_launcher, QLauncher):
           task_command.show(task_specifier.copy(), queue_cmd_only=task_launcher.is_dynamic_qsub)
        ...
        if task_command.allowed_list != []:
            ...
            if not (all_keys <= allowed_list):
                ...
                print "Keys %s not in the allowed list of the TaskCommand!" % list(clashes)
                sys.exit()

        response = self.input_options(['y', 'n','quit','savefile'], '\nShow available taskcommand entries?')
        if response == 'quit': sys.exit()
        if response == 'y':  
           task_command.show(task_specifier.copy())
        if response == 'savefile':
            fname = raw_input('Filename: ').replace(' ','_')
            with open(fname,'w') as f: task_command.show(task_specifier.copy(),file_handle=f)

    def review_task_launcher(self, task_launcher,task_command):
        print self.section('TaskLauncher')
    ...
    def __call__(self,f):
       ...
       if f.__module__ == '__main__':
          ...
          if not isinstance(task_launcher, TaskLauncher): 
             print "Function must return a Tasklauncher not '%s'" % str(task_launcher); return
                
          if self.review:
             print_level = param.parameterized.min_print_level
             param.parameterized.min_print_level = 0
             ...
             if response == 'y':  task_launcher.dispatch()
             param.parameterized.min_print_level = print_level
          else: task_launcher.dispatch()
       return f
