...
CLUSTER = True
if CLUSTER:  Launcher = QLauncher; batch_name = 'topo_cluster'; max_concurrency = None
else:        Launcher = TaskLauncher; batch_name = 'topo_local'; max_concurrency = 2
...
def topo():
    ...
    # Note that 'topo_local' is the batch_name
    tasklauncher = Launcher(batch_name,combined_spec,run_batch_command)
    tasklauncher.exit_callable = exit_function

    # Launcher options
    tasklauncher.print_info = 'stdout'  # Alternatively: 'summary', 'silent'
    tasklauncher.tag = 'example_tag'
    tasklauncher.max_concurrency = max_concurrency # The default for local (you can change it). Ignored for cluster.
    return tasklauncher
